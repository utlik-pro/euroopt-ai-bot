...
def load_stores_minsk():
    ...
    for i, st in enumerate(items):
        ...
        if isinstance(sched, dict) and sched:
            ...
            hrs = st.get("hours", "")
        # Жирный текст карточки: больше ключевых слов = выше embedding-score
        # для запросов вида «магазины Евроопт в X», «адрес магазина в Y», «гипермаркет в Z».
        text = (
            f"Адрес магазина Евроопт в городе {city}\n"
            f"Магазин Евроопт ({fmt}) — продуктовый магазин сети Евроторг.\n"
            f"Город: {city}\n"
            f"Полный адрес: {addr}\n"
            f"Улица: {street}\n"
            f"Дом: {house}\n"
            f"Формат торговой точки: {fmt} (Евроопт {fmt})\n"
            f"Часы работы (режим, время работы, расписание): {hrs}\n\n"
            f"Ключевые слова: магазин Евроопт, адрес магазина, продуктовый магазин, "
            f"супермаркет, гипермаркет, маркет, минимаркет, торговая сеть Евроторг, "
            f"режим работы, часы работы, время работы, как добраться, "
            f"г. {city}, {city}, {street}, {fmt}, {fmt} в {city}"
        )
...
def load_stores_hit_groshyk():
    ...
    for i, st in enumerate(items):
        ...
        fmt = st.get("format", "")
        street = st.get("street", "")
        # Описание сети (детальное → выше embedding score для запросов про дискаунтеры)
        if brand == "Хит":
            desc = "сеть магазинов низких цен «Хит! Дискаунтер» (hitdiscount.by) — дискаунтер"
        else:
            desc = "сеть магазинов жёсткого дискаунтера «Грошык» (groshyk.by) — дискаунтер"
        text = (
            f"Адрес магазина {brand} в городе {city}\n"
            f"Магазин {brand} — {desc}.\n"
            f"Сеть: {brand} (часть холдинга Евроторг)\n"
            f"Город: {city}\n"
            f"Полный адрес: {addr}\n"
            f"Улица: {street}\n"
            f"Регион: {region}\n"
            f"Формат торговой точки: {fmt}\n\n"
            f"Ключевые слова: магазин {brand}, адрес магазина {brand}, дискаунтер, "
            f"низкие цены, продуктовый магазин, торговая сеть, "
            f"г. {city}, {city}, {region}, {street}, {brand} в {city}"
        )
...
def load_stores_avtolavki():
    ...
    for i, st in enumerate(items):
        ...
        street = st.get("street", "")
        text = (
            f"Автолавка Евроопт в населённом пункте {city}\n"
            f"Формат: мобильный магазин на колёсах (автолавка) сети Евроопт.\n"
            f"Автолавка — это передвижной магазин, который приезжает в небольшие "
            f"населённые пункты по графику.\n"
            f"Локация: {addr}\n"
            f"Улица: {street}\n\n"
            f"Ключевые слова: автолавка, автомагазин, мобильный магазин, "
            f"передвижной магазин, выездная торговля, магазин на колёсах, "
            f"село, деревня, агрогородок, посёлок, г. {city}, {city}, "
            f"автолавка Евроопт в {city}, расписание автолавки, маршрут автолавки"
        )
