...
def sanitize_context_text(text: str, max_chars: int = 2000) -> str:
    ...
    NB: ПДн тут НЕ маскируются намеренно — телефоны горячих линий Евроопта,
    ФИО публичных лиц (генеральный директор и т.п.), адреса магазинов —
    публичная информация, должна доходить до LLM и до пользователя в ответах.
    PII-фильтр применяется только к сообщениям ПОЛЬЗОВАТЕЛЯ (см. pipeline.py).
    """
    if not text:
        return ""
